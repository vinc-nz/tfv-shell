...
local function tabline(options)
    ...
    for index = 1, fn.tabpagenr('$') do
        ...
        -- pwd
        local abspath = fn.getcwd(winnr, index)
        local global_workdir = fn.getcwd(-1, 1)
        if abspath == global_workdir then
            s = s .. '[' .. fn.fnamemodify(abspath, ":t") .. '] '
        else
            local relpath = abspath:gsub(global_workdir .. '/', "")
            s = s .. '[' .. relpath .. '] '
        end
        -- modify indicator
        if
